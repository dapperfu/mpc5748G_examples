...
#define ADC_VREF                                                               \
  3300           /* ADC ref voltage for both ADC modules. 3300mV or 5000mV */
uint16_t Result; /* ADC channel conversion results */
uint16_t ResultInMv; /* ADC channel conversion results in mV */

void ADC1_PadConfig_ChanSelect(
    void) { /* Configures ADC pads & select scan chans */
...
void ADC1_Calibration(void) { /* Calibrate to compensate for variations.
                                                                         Steps
                                 below are from reference manual */
  uint32_t __attribute__((unused)) ADC1_Calibration_Failed =
      1; /* Calibration has not passed yet */
...
void SAR_ADC1_Init(void) /* Initialize ADC_1 in Cross Trigger mode */
{
  /* 12-bit ADC */
  ADC_1.MCR.B.CTU_MODE = 1; /* CTU mode is ON */
  ADC_1.MCR.B.CTUEN = 1;    /* CTU Enable */
  ADC_1.MCR.B.ADCLKSEL =
      1;                 /* ADC clock frequency is equal to bus clock = 80MHz */
  ADC_1.MCR.B.OWREN = 1; /* Data Overwrite enable */

  ADC_1.MCR.B.PWDN = 0; /* Enable SAR_ADC */
}
