 ...
 */
/*				 and BCTU triggers ADC1_CH9 conversion. The
 * result will	     */
/*				 blink user LEDs (according to its value) after
 * each
 */
/*				 conversion.
 */
...
/* 1.1	  K Shah			  18 Mar 2016	Ported program to make
 * compatible*/
/* 							  with S32DS &
 * DEVKIT-MPC5748G
 */
...
void peri_clock_gating(
    void); /* Configure gating/enabling peripheral(ADC, PIT, BCTU) clocks */
...
__attribute__((section(".text"))) int main(void) {
  ...
  peri_clock_gating(); /* Configure gating/enabling peripheral(ADC, PIT, BCTU)
                          clocks for modes*/
  ...
  while (1) {
    if (BCTU.MSR.B.NDATA1) { /* If normal scan channels finished converting */
      ...
      for (i = 0; i < 655350; i++)
        ; /* Wait for some random time but less than PIT overflow time */
...
void peri_clock_gating(void) { /* Configure gating/enabling peripheral(ADC, PIT,
                                  BCTU) clocks for modes*/
  MC_ME.RUN_PC[0].R = 0x00000000; /* Gate off clock for all RUN modes */
  MC_ME.RUN_PC[1].R =
      0x000000FE; /* Enable peripheral clock for all RUN modes */
                  /* Note: RUN_PC0 is default configured for PCTLs */
  MC_ME.PCTL[25].B.RUN_CFG =
      0x1; /* ADC1: select peripheral configuration to RUN_PC[1] */
  MC_ME.PCTL[91].B.RUN_CFG =
      0x1; /* PIT: select peripheral configuration RUN_PC[1] */
  MC_ME.PCTL[0].B.RUN_CFG =
      0x1; /* BCTU: select peripheral configuration RUN_PC[1] */
}
