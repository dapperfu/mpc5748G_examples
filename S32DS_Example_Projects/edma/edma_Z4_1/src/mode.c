/*****************************************************************************/
/* FILE NAME: mode.c                           		 COPYRIGHT (c) NXP
 * 2016  */
/*                                                      All Rights Reserved  */
...
/* 1.1    S Mihalik			  11 Sep 2013   Added mode verfication
 * after mode*/
/*                    		  transition, added comments and deleted
 * unused  */
/* 							  functions
 */
/* 1.2	  S Mihalik			  08 Oct 2013   Re-organization,minor
 * code change*/
/* 1.3    S Mihalik  		  13 Nov 2013   Replaced "while" with "if"
 * clause*/
...
void PLL_160MHz(void) {
  ...
  */
  PLLDIG.PLLDV.B.PREDIV = 1; /* Divide input clock by 1, resulting clock will be
                                reference input for the PLL analog loop */
  PLLDIG.PLLDV.B.MFD = 16;   /* Loop multiplication factor 16*40 MHz */
  PLLDIG.PLLDV.B.RFDPHI = 1; /* VCO post divider = 4 so 16*40/40 = 160MHz */

  PLLDIG.PLLCAL3.R = 0x09C3C000; /* Contains the denominator of the fractional
                                    loop division factor */
                                 /* MFDEN = 9C3C000h = 163823616 */
  PLLDIG.PLLFD.B.SMDEN = 1;      /* Sigma delta modulation enabled */
  /* switch to PLL */
  MC_ME.DRUN_MC.R = 0x00130072; /* MVRON = 1, FLAON = RUN mode, PLLON=1, FXOSCON
                                   = 1, FIRCON = 1, SYSCLK = PLL PHI_0 */
