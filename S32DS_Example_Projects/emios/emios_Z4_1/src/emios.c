...
void init_emios_global_prescalers(void) {
  ...
  eMIOS_0.MCR.B.FRZ =
      0; /* No Freeze channel registers in debug mode if channel FREN=0 */
...
void emios0_ch23_OPWFMB(
    void) { /* Configures eMIOS 0 channel 23: OPWFMB, 200 count*/
  ...
  eMIOS_UC_0.UC[23].CNT.R =
      1; /* OPWFMB start counter between 1 & B register value*/
  eMIOS_UC_0.UC[23].C.B.EDPOL = 1; /* Output polarity on A match */
  eMIOS_UC_0.UC[23].C.B.MODE =
      0x58; /* Output Pulse Width & Frequency Modulation*/
...
void emios1_ch06_IPWM(void) {   /* Configures eMIOS 1 channel 06: IPWM */
  ...
  SIUL2.IMCR[42].B.SSS =
      2; /* eMIOS1 channel 6, E1UC_6_H : connected to pad PH4 */
}
