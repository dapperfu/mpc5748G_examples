...
/*				MCB: eMIOS1_CH23 is configured to count up to
 * 1000, hence 	 */
/*				it demos a timer to generate trigger at 1000/1MHz
 * = 1us		 */
/*				OPWMB: eMIOS1_CH18 is configured to generate the
 * waveform 	 */
/*				with 25% duty cycle on PJ4 (DS9 LED)
 */
/*				OPWFMB: eMIOS0_CH23 is configured to generate
 * waveform with	 */
/*				200us(200 cycles) period and 10%(20 cycles) duty
 * cycle
 */
/*				on pin PF6(J13_6)
 */
/*				IPM: eMIOS1_CH05 is configured to measure the
 * period of input*/
/*				waveform on PH3(PJ4_7)
 */
/*				IPWM: eMIOS1_CH06 is configured to measure the
 * duty cycle	 */
...
#define MAXCNT                                                                 \
  1000 /* Count value for MCB channel. (used for OPWMB, IPM, IPWM)*/

extern void xcptn_xmpl(void);
void peri_clock_gating(
    void); /* Configure gating/enabling peripheral(eMIOS0, eMIOS1) clocks */
...
int main(void)
{
  ...
  peri_clock_gating(); /* Configure gating/enabling peripheral clocks(eMIOS0,
                          eMIOS1) for modes*/
                       /* Configuration occurs after mode transition */
  system160mhz();      /* sysclk=160MHz, dividers configured, mode transition*/

  init_emios_global_prescalers(); /* Initialize eMIOS 1,2: pre-scale
                                     80MHz/80=1MHz */

  emios1_ch23_MCB();    /* Configures eMIOS 1 channel 23: MCB, 1K count (1K usec
                           period) */
  emios1_ch18_OPWMB();  /* Configures eMIOS 1 channel 18: OPWMB with time base
                           eMIOS1 ch0 */
  emios0_ch23_OPWFMB(); /* Configures eMIOS 0 channel 23: OPWFMB 200 counts-200
                           us period*/
  emios1_ch05_IPM();    /* Configures eMIOS 1 channel 5:  IPM   with time base
                           eMIOS1 ch0 */
  emios1_ch06_IPWM();   /* Configures eMIOS 1 channel 6:  IPWM  with time base
                           eMIOS1 ch0 */
...
void peri_clock_gating(void) {    /* Configure gating/enabling peripheral
                                     clocks(eMIOS0, eMIOS1) for modes*/
