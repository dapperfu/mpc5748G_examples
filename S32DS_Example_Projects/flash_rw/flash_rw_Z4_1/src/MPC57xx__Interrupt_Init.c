...
#define mfspr(rn)                                                              \
  ({                                                                           \
    unsigned int rval;                                                         \
    __asm__ volatile("mfspr %0," stringify(rn) : "=r"(rval));                  \
    rval;                                                                      \
  })
...
#define INTC_BCR_HVEN(coreId)                                                  \
  ((uint32_t)(0x1UL << ((coreId) << 4UL))) /* coreId - hw coreId (value that   \
                                              is read from PIR) */
...
/**************************************************************************/
void SetIVPR(register unsigned int x) { __asm__("mtIVPR %0 \n\t" : : "r"(x)); }
