...
#define INTC_BCR_HVEN(coreId)                                                  \
  ((uint32_t)(0x1UL << ((coreId) << 3UL))) /* coreId - hw coreId (value that   \
                                              is read from PIR) */
...
/**************************************************************************/
void SetIVPR(register unsigned int x) { MTSPR(63, x); }
...
void InitIVORS(void) {
  ...
  PPCASM("mtspr       IVOR9, r3"); /* IVOR9 409 Auxiliary processor unavailable.
                                      Not used by the e200z6. */
  ...
  PPCASM("mtspr       IVOR33, r3"); /* IVOR33 529 SPE floating-point data
                                       exception                       */
  PPCASM("e_add16i    r3, r0, 0x120");
  PPCASM("mtspr       IVOR34, r3"); /* IVOR34 530 SPE floating-point round
                                       exception                      */
#else
...
void InitINTC(void) {
  ...
  /* Set INTC ISR vector table base addr. */
  if (coreId)
    INTC.IACKR[1].R = (uint32_t)&IntcIsrVectorTable[0];
  else
