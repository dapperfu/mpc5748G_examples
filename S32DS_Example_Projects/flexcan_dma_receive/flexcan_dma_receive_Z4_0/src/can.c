...
void initCAN_0_rx(void) { /* General init.  MB IDs: MB4 ID_STD=0x555 */
  ...
  } /* Wait for freeze acknowledge to set */
  ...
  CAN_0.CTRL2.B.RFFN =
      0x0; /* Message buffers 0-7 are occupied by RX FIFO and ID filter table */
  /* There are 8 ID filter table elements */

  CAN_0.MCR.B.IDAM =
      0x0; /* One full ID (standard and extended) per ID Filter Table element */

  /* Initialize  ID filter table elements 0-7 to receive messages with standard
   * ID 0x555 (RTR and IDE = 0) */
