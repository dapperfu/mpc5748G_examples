...
void DMA_Init(void) {
  ...
  EDMA.TCD[DMA_CHANNEL].SADDR.R =
      CAN0_BASE_ADDRESS + 0x80;           /* Load Address of Source data */
  ...
  EDMA.TCD[DMA_CHANNEL].SLAST.R =
      0; /* No address Adjustment after major loops */

  EDMA.TCD[DMA_CHANNEL].DADDR.R =
      (unsigned int)&RxDATA[0];           /* Destination Address */
  EDMA.TCD[DMA_CHANNEL].ATTR.B.DSIZE = 2; /* Write 2**0 = 1 byte per transfer */
  EDMA.TCD[DMA_CHANNEL].ATTR.B.DMOD =
      0;                            /* Destination modulo feature not used  */
  EDMA.TCD[DMA_CHANNEL].DOFF.R = 4; /* Increment destination addr by 1 */
  EDMA.TCD[DMA_CHANNEL].DLASTSGA.R =
      0; /* No addr adjustment after major loop */

  EDMA.TCD[DMA_CHANNEL].CSR.B.DREQ = 1; /* Disable Request */
  EDMA.TCD[DMA_CHANNEL].NBYTES.MLNO.B.NBYTES =
      16; /* NBYTES - Transfer 1 byte per minor loop */
  /* TCD Current Minor Loop Link, Major Loop Count (Channel Linking Disabled) */
  EDMA.TCD[DMA_CHANNEL].CITER.ELINKNO.B.ELINK =
      0; /* Enable channel-to-channel linking on minor-loop complete */
  EDMA.TCD[DMA_CHANNEL].CITER.ELINKNO.B.CITER =
      1; /* Current Major Iteration Count */
  /* TCD Beginning Minor Loop Link, Major Loop Count (Channel Linking Disabled)
   */
  EDMA.TCD[DMA_CHANNEL].BITER.ELINKNO.B.ELINK =
      0; /* Enables channel-to-channel linking on minor loop complete */
  EDMA.TCD[DMA_CHANNEL].BITER.ELINKNO.B.BITER =
      1; /* Starting Major Iteration Count */
  EDMA.TCD[DMA_CHANNEL].CSR.B.MAJORELINK =
  ...
  EDMA.TCD[DMA_CHANNEL].CSR.B.INTHALF =
      0; /* Disable an interrupt when major counter is half complete. */
  EDMA.TCD[DMA_CHANNEL].CSR.B.INTMAJOR =
      1; /* Disable an interrupt when major iteration count completes */
