...
/*				Project Continuously transmits the message "Hello"
 * to the 	 */
/* 				CAN0 (P5) port.
 */
/* 				Can be used with receive project on another
 * board.
 */
/*				Or observe the waveforms on pins of port P5 on
 * board.
 */
/*                                                                           */
/*****************************************************************************/
...
void peri_clock_gating(
    void); /* Configure gating/enabling peripheral(CAN) clocks */
...
int main(void)
{
  ...
  peri_clock_gating(); /* Configure gating/enabling peripheral clocks(CANs) for
                          modes*/
  ...
  while (1) {
    ...
    SIUL2.MSCR[PA10].B.OBE =
        1; /* Pad PA10 (10): OBE=1. On EVB enable low DS4 LED */
  }
...
void peri_clock_gating(
    void) { /* Configure gating/enabling peripheral clocks(CANs) for modes*/
  MC_ME.RUN_PC[0].R = 0x00000000; /* Gate off clock for all RUN modes */
  MC_ME.RUN_PC[1].R =
      0x000000FE; /* configures peripheral clock for all RUN modes */

  MC_ME.PCTL[70].B.RUN_CFG =
      0x1; /* FlexCAN 0: select peripheral configuration RUN_PC[1] */
}
