/*****************************************************************************/
/* FILE NAME: platform_inits.c                 COPYRIGHT (c) Freescale 2013  */
/*                                                      All Rights Reserved  */
/* DESCRIPTION: Various MPC5748G platform initializations including:         */
/*       - RAM, Flash configuration: wait states per frequency, etc.         */
/*       - Crossbar configuration                                            */
/*                                                                           */
/*****************************************************************************/
/* REV      AUTHOR        DATE        DESCRIPTION OF CHANGE                  */
/* ---   -----------    ----------    ---------------------                  */
/* 1.0	  SM            01 Aug 2013   Initial Version                        */
/*****************************************************************************/

#include "platform_inits.h"
#include "project.h"
...
#define PFLASH_PFCR1_VALUE_160MHz                                              \
  0x00152417 /* Flash port p0 configuration:                                   \
              */
...
#define PFLASH_PFCR3_VALUE_BASIC                                               \
  0x00000000 /* Flash way cfg, arb., BAF dis.                                  \
              */
...
void memory_config_16mhz(void) {
  ...
  uint32_t mem_write_code_vle[] = {
      ...
      0x00040004  /* 2 se_blr's instr.: branches to return address in link
                     reg */
  };
  ...
  (*((mem_write_code_ptr_t)mem_write_code_vle)) /* PFCR2 initialization */
      (PFLASH_PFCR2_VALUE_BASIC, (uint32_t)&PFLASH_PFCR2_REG);

  (*((mem_write_code_ptr_t)mem_write_code_vle)) /* PFCR3 initialization */
      (PFLASH_PFCR3_VALUE_BASIC, (uint32_t)&PFLASH_PFCR3_REG);

  (*((mem_write_code_ptr_t)mem_write_code_vle)) /* PFAPR initialization */
      (PFLASH_PFAPR_VALUE_BASIC, (uint32_t)&PFLASH_PFAPR_REG);

  (*((mem_write_code_ptr_t)mem_write_code_vle)) /* PFCR4 initialization */
      (PFLASH_PFCR4_VALUE_BASIC, (uint32_t)&PFLASH_PFCR4_REG);
...
void memory_config_160mhz(void) {
  ...
  (*((mem_write_code_ptr_t)mem_write_code_vle))(PFLASH_PFCR2_VALUE_BASIC,
                                                (uint32_t)&PFLASH_PFCR2_REG);

  (*((mem_write_code_ptr_t)mem_write_code_vle))(PFLASH_PFCR3_VALUE_BASIC,
                                                (uint32_t)&PFLASH_PFCR3_REG);

  (*((mem_write_code_ptr_t)mem_write_code_vle))(PFLASH_PFAPR_VALUE_BASIC,
                                                (uint32_t)&PFLASH_PFAPR_REG);

  (*((mem_write_code_ptr_t)mem_write_code_vle))(PFLASH_PFCR4_VALUE_BASIC,
...
void crossbar_config(void) { /* Place holder for code */
}
