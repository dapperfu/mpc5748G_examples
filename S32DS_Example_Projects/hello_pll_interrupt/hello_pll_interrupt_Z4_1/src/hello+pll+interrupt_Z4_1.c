 ...
 */
/*				to the interrupts. PIT0 generates the interrupt
 * in Core 0 and 	*/
/* 				toggles the DS4 LED at every 1s. After every 4
 * PIT0 interrupts	*/
/* 				it generates the software interrupt in core 1
 * which toggles		*/
/* 				LED DS7 at every 4s.
 */
/* 				Core 1 also generates PIT1 interrupt which
 * toggles
 */
/* 				LED DS5 at every 0.5s.
 */
/* 				Core 2 also generates PIT2 interrupt which
 * toggles
 */
...
void peri_clock_gating(
    void); /* Configuration gating/enabling peripheral clocks for modes*/
...
__attribute__((section(".text"))) int main(void) {
  ...
  // Since We are using PIT- one of the peripherals. We need to enable
  // peripheral clocks.
  peri_clock_gating(); /* Configure gating/enabling peripheral(PTI) clocks for
                          modes*/
  ...
  initGPIO(); /* Initializes LED, buttons & other general purpose pins for NXP
                 EVB */
