...
/*				 Master sends the address and data and Slave
 * receives both   */
/*				 The address and the data
 */
/*				 Provide a pull-up of 3.3V via pull-up
 * register(10k) to both */
/* 				 SCL and SDA lines
 */
/*				 	I2C_1
 * I2C_2
...
void peri_clock_gating(
    void); /* Configure gating/enabling peripheral(I2C) clocks */
  ...
  while (1) {
    ...
    if (data == 'H') {
      SIUL2.MSCR[PA10].B.OBE ^=
          1; /* Pad PA10 (10): OBE=OBE xor 1. On EVB active low DS4 LED */
      data = 0;
    }
  }
  return 0;
...
void peri_clock_gating() { /* Configure gating/enabling peripheral(I2C) clocks
  ...
  MC_ME.PCTL[31].B.RUN_CFG =
      0x1; /* I2C_1: select peripheral configuration RUN_PC[1] */
  MC_ME.PCTL[32].B.RUN_CFG =
      0x1; /* I2C_2: select peripheral configuration RUN_PC[1] */
}
