 ...
 */
/* 				at PC8 and PC9 respectively. It is integrated
 * with openSDA 	 */
/* 				interface. So use openSDA micro-usb interface
 */
/*				to connect to computer
 */
/*				Example sends the welcome message in the beginning
 * of
 */
/*				the program. Then receives the character and sends
 * the
 */
...
void peri_clock_gating(
    void); /* Configures gating/enabling peripheral(LIN) clocks */
...
int main(void)
{
  ...
  peri_clock_gating(); /* Configures gating/enabling peripheral clocks for
                          modes*/
                       /* Configuration occurs after mode transition */
  system160mhz();      /* sysclk=160MHz, dividers configured, mode transition*/

  initLINFlexD_2(80,
                 57600); /* Initialize LINFlex2: UART Mode 80MHz, 57600 Baud */
  testLINFlexD_2();      /* Display a message on the terminal */
  //  	initLINFlexD_1( 80, 57600 );/* Initialize LINFlex1: UART Mode 80MHz,
  //  57600 Baud */
  //  	testLINFlexD_1();			/* Display a message on the
  //  terminal
  //  */
  ...
  while (1) {
    ...
    //  		  Input = rxLINFlexD_1();	/* Wait for & receive
    //  one byte */ 		  txLINFlexD_1( Input );	/* Transmit one
    //  byte */
  }
...
void peri_clock_gating(
    void) { /* Configures gating/enabling peripheral clocks for modes*/
  MC_ME.RUN_PC[0].R = 0x00000000; /* Gate off clock for all RUN modes */
  MC_ME.RUN_PC[1].R =
      0x000000FE; /* Configures peripheral clock for all RUN modes */
  MC_ME.PCTL[52].B.RUN_CFG =
      0x1; /* LINFlex 2: select peripheral configuration RUN_PC[1] */
  //  MC_ME.PCTL[51].B.RUN_CFG = 0x1;  	/* LINFlex 2: select peripheral
  //  configuration RUN_PC[1] */
}
