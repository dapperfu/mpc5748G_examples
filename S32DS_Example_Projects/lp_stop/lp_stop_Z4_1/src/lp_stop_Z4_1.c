 ...
 */
/* 				At RTC timeout(after 0.1s), the stop mode is
 * exited and		 */
/* 				DS4 LED is turned off and again goes to the stop
 * mode.		 */
/* 				Again at RTC timeout(after 0.9s), the stop mode
 * is exited	 */
...
void peri_clock_gating(
    void); /* ConfigureS gating/enabling peripheral(WKPU, RTC-API) clocks */
...
int main(void)
{
  ...
  peri_clock_gating(); /* Configures gating/enabling peripheral(WKPU, RTC-API)
                          clocks for modes*/
 ...
 MC_ME.ME: enables all modes for Mode Entry module
  ...
  MC_ME.RUN_MC[3].R =
      0x001F0090; /* MVRON=1 FLAON=RUN SIRCON=1 FIRCON=1 sysclk=FIRC */
  MC_ME.STOP_MC.R =
  ...
  while (MC_ME.GS.B.S_CURRENT_MODE != 7) {
  } /* Verify RUN3 (0x7) is the current mode */
  ...
  SIUL2.MSCR[PA10].B.OBE =
      1; /* Pad PA10 (10): OBE=1. On EVB active low DS4 LED */
  ...
  while (1) {
    ...
    while (MC_ME.GS.B.S_CURRENT_MODE != 7) {
    } /* Verify RUN3 (0x7) is current mode */
...
void peri_clock_gating(void) {    /* Configures gating/enabling peripheral(WKPU,
                                     RTC-API) clocks for modes*/
  MC_ME.RUN_PC[0].R = 0x00000000; /* gate off clock for all RUN modes */
  MC_ME.RUN_PC[1].R =
      0x000000FE; /* Configures peripheral clock for all RUN modes */
  MC_ME.RUN_PC[7].R = 0x00000088; /* Run peripheral configuration 7 settings:
                                     run in DRUN, RUN3 modes */
  MC_ME.LP_PC[7].R =
      0x00000400; /* LP peripheral configuration 7 settings: run in STOP  */

  MC_ME.PCTL[93].R =
      0x3F; /* WKPU: select peripheral configuration RUN_PC[7], LP_PC[7] */
  MC_ME.PCTL[102].R =
      0x3F; /* RTC-API: select peripheral configuration RUN_PC[7], LP_PC[7] */
}
