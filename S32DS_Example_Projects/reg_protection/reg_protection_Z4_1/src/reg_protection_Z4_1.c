...
/*				SOFT LOCK: This part writes the value to the
 * mirror registers*/
/*				and locks the register. So attempt to modify
 * original
 */
/* 				register or mirror register will fail.
 */
/*				CLEAR SOFT LOCK: Clears the soft-lock bit. So
 * now
 */
/*				it can modify register value
 */
/*				HARD LOCK: Locks the soft-lock bit by setting
 * hard lock. 	 */
/*				So any attempt to modify original register,
 * mirror register  */
/*				or soft lock bit will generate machine check
 * interrupt.		 */
/*                                                                           */
/*****************************************************************************/
...
int main(void) {
  ...
  uint32_t i = 0;
  uint32_t __attribute__((unused)) LockedRegisterUnchanged =
      0; /* test result */
  uint32_t __attribute__((unused)) UnlockedRegisterChanged =
      0;                                                       /* test result */
  ...
  SIUL2_GLOBALLOCK.GCR.B.HLB =
      1; /* Protect all protectable registers lock bits*/
  /* Next instruction will cause a Machine Check on cut 2 or later silicon */
  SIUL2_LOCKBITS.SSLB[14].R =
      0xF0; /* IFERO: Attempt to remove protection -> IVOR1 */
  /* Next instruction will cause a Machine Check on cut 2 or later silicon */
  SIUL2.IFER0.R = 0x66666666; /* IFERO: Attempt new value -> IVOR1 */
  /* Next instruction will cause a Machine Check on cut 2 or later silicon */
  SIUL2_REGLOCK.IFER0.R =
      0x66666666; /* IFERO: Attempt new value with lock -> IVOR1 */
