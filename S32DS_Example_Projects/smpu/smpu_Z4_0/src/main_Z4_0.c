 ...
 */
/* 				Unit(SMPU). Locks the last 4kB of RAM to allow
 * just read	 */
/* 				access. So any attempt to write to those
 * locations will		 */
 ...
 */
/* 1.3	  K Zemskov			  28 Dec 2016   Updated for RAM
 * Configuration	 */
/*****************************************************************************/
...
__attribute__((section(".text"))) int main(void) {
  ...
  *(unsigned int *)0x400bf000 = 0xAAAAAAAA;
  ...
  initSMPU(); /* Initialize the SMPU and particularly configured to read only
                 access to last 4kB of SRM memory region */

  /* Write within last 4kB SRAM - this access will trigger exception due to SMPU
   * access violation */
  *(unsigned int *)0x400bf000 = 0x55555555;
  ...
  uint32_t __attribute__((unused)) *mem_ptr = (uint32_t *)0x400bf000;
