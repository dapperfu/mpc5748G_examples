 ...
 * slave mode.	 */
/*				Master continuously sends data to Slave and
 * Slave replies	 */
/* 				to master.
 */
/*                                                                           */
...
void peri_clock_gating(
    void); /* Configures gating/enabling peripheral(DSPI, SPI) clocks */
...
int main(void)
{
  ...
  peri_clock_gating(); /* Configures gating/enabling peripheral(DSPI, SPI)
                          clocks for modes*/
  ...
  while (1) {
    SPI_0.PUSHR.PUSHR.R =
        0x00001234; /* Initialize slave SPI_0's response to master */
    DSPI_3.PUSHR.PUSHR.R =
        0x08015678; /* Transmit data from master DSPI to slave SPI with EOQ */
...
void peri_clock_gating(
  ...
  MC_ME.PCTL[43].B.RUN_CFG =
      0x1; /* DSPI 2: select peripheral configuration RUN_PC[1] */
  MC_ME.PCTL[96].B.RUN_CFG =
      0x1; /* SPI 0: select peripheral configuration RUN_PC[1] */
}
