...
/*				Configures DSPI_3 in Master mode and SPI_0 in
 * slave mode.	 */
/*				In Master DMA sends data to DSPI transfer register
 * and
 */
...
void peri_clock_gating(
    void); /* Configures gating/enabling peripheral(SPI, DSPI, DMAMUX) clocks */
void bridges_config(void); /* Bridge configuration */
void crossbar_config_DMA_highest(
    void); /* Crossbar priority configuration for DMA */
...
int main(void)
{
  ...
  peri_clock_gating(); /* Configures gating/enabling peripheral(SPI, DSPI,
                          DMAMUX) clocks for modes */
...
void peri_clock_gating(void) {    /* Configures gating/enabling peripheral(SPI,
                                     DSPI, DMAMUX) clocks for modes */
  MC_ME.RUN_PC[0].R = 0x00000000; /* gate off clock for all RUN modes */
  MC_ME.RUN_PC[1].R =
      0x000000FE; /* configures peripheral clock for all RUN modes */
  MC_ME.PCTL[43].B.RUN_CFG =
      0x1; /* DSPI_3: select peripheral configuration RUN_PC[1] */
  MC_ME.PCTL[96].B.RUN_CFG =
      0x1; /* SPI_0: select peripheral configuration RUN_PC[1] */
  MC_ME.PCTL[90].B.RUN_CFG =
      0x1; /* DMAMUX: select peripheral configuration RUN_PC[1] */
}
...
/* Configures desired crossbar priorities for crossbar slaves to give to
 * crossbar masters. */
void crossbar_config_DMA_highest(void) {
  AXBS_0.PORT[6].PRS.R =
      0x70654321; /* PBridge 0:    gives highest priority to DMA */
  AXBS_0.PORT[5].PRS.R =
      0x70654321; /* PBridge 1:    gives highest priority to DMA */
  AXBS_0.PORT[2].PRS.R =
      0x70654321; /* Flash Port 2: gives highest priority to DMA */
}
