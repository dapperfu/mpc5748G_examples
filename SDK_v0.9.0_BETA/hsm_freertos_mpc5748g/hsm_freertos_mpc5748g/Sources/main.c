        ...
        if (xQueueReceive(g_tQueueHandle, &tRxData, (TickType_t)TASK1_DELAY))
        {
            switch (eEncType)
            {
                ...
                case ENCRYPT_ECB:
                    ...
                    if ((stat == STATUS_SUCCESS) && (strcmp((char*)ucMsg, PLAINTEXT) == 0))
                    {
                    ...
                    break;
                case ENCRYPT_CBC:
                    HSM_DRV_DecryptCBC(HSM_RAM_KEY, (uint8_t*)tRxData.ucEncMsg, MESSAGE_LENGTH, (uint8_t*)tRxData.ucInitVector, (uint8_t*)ucMsg, TIMEOUT_ENCRYPTION);
                    eEncType = ENCRYPT_NONE;
                    /* Send the status to the console */
                    if ((stat == STATUS_SUCCESS) && (strcmp((char*)ucMsg, PLAINTEXT) == 0))
                    {
