...
/* 1.3	  K Shah			  18 Mar 2016	Ported program to make
 * compatible*/
...
void peri_clock_gating(
    void);              /* Configure gating/enabling peripheral(ADC) clocks */
...
int main(void)
{
  ...
  system160mhz();      /* Sets clock dividers= max frequency, calls PLL_160MHz
                          function which:      MC_ME.ME: enables all modes for Mode Entry
                          module      Connects XOSC to PLL      PLLDIG: LOLIE=1,
                          PLLCAL3=0x09C3_C000, no sigma-delta, 160MHz      MC_ME.DRUN_MC:
                          configures sysclk = PLL      Mode transition: re-enters DRUN
                          mode */
  hw_init();           /* Enable all Cores in all run modes */
...
void peri_clock_gating(
  ...
  MC_ME.PCTL[25].B.RUN_CFG =
      0x1; /* ADC1: Select peripheral configuration RUN_PC[1] */
}
